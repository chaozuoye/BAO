...
/*    //房间棋盘信息
    ...
    //最大观察者数量
    public const int MAX_OBSERVER_AMOUNT = 200;
...
{
    public static Dictionary<Player, Move> PlayerPosition;
    ...
    private static void _Receive(object obj)
    {
        ...
        while (true)
        {
            ...
            {
                Console.WriteLine($"1{client.RemoteEndPoint}已掉线:{ex.Message}");
                player.Offline();
                return;
            }
            ...
            {
                Console.WriteLine($"2{client.RemoteEndPoint}已掉线");
                player.Offline();
                return;
            }
            ...
            using (MemoryStream stream = new MemoryStream(data))
            {
                ...
                try
                {
                    ...
                    Console.WriteLine(type+" length:"+length);
                }
                catch (Exception)
                {
                    Console.WriteLine($"3{client.RemoteEndPoint}已掉线");
                    player.Offline();
                    return;
                }
            }
            ...
            if (length - 8 > 0)
            {
                ...
                if (receive < data.Length)
                {
                    Console.WriteLine($"4{client.RemoteEndPoint}已掉线");
                    player.Offline();
                    return;
                }
            }
    ...
    public static void Start(string ip)
    {
        ...
        PlayerPosition = new Dictionary<Player, Move>();
    ...
    public static void Offline(this Player player)
    {
        ...
        byte[] data = NetworkUtils.Serialize(result);
        //通知其他玩家
        foreach(var each in Players)
        {
        ...
        {
            Rooms[player.RoomId].Players.Remove(player);
            //如果该玩家此时的房间人数=0则将房间删除
            if (Rooms[player.RoomId].Players.Count <= 0)
            {
